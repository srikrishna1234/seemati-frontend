...
function installGlobalHandlers() {
  ...
  window.addEventListener('error', function(evt){
    try {
      console.error('[window.error] message:', evt.message, 'source:', evt.filename, 'lineno:', evt.lineno, 'col:', evt.colno, 'error:', evt.error);
  ...
  window.onerror = function(msg, url, lineNo, columnNo, error){
    try {
      console.error('[onerror] msg:', msg, 'url:', url, 'line:', lineNo, 'col:', columnNo, 'err:', error);
...
(function showBootBadge(){
  try {
    ...
    Object.assign(b.style, {position:'fixed', right:8, bottom:8, zIndex:999998, background:'#000', color:'#fff', padding:'6px 8px', fontSize:'12px', borderRadius:'4px', opacity:0.85});
    document.documentElement.appendChild(b);
  } catch(e){}
})();

// Render the app inside a BrowserRouter so hooks like useNavigate work everywhere
