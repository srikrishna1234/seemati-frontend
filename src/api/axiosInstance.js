...
instance.interceptors.request.use(
  (req) => {
    try {
      ...
      if (req && typeof req.url === "string" && /^\/api(\/|$)/i.test(req.url)) {
        const oldUrl = req.url;
        req.url = req.url.replace(/^\/api/i, "") || "/";
        if (process.env.NODE_ENV !== "production") console.debug("[axios] rewrite url:", oldUrl, "->", req.url);
      }
...
if (process.env.NODE_ENV !== "production") {
  instance.interceptors.response.use(
    ...
    (err) => {
      ...
      if (err.response) { const msg = err.response.data?.message || err.response.statusText || `HTTP ${err.response.status}`; return Promise.reject(new Error(msg)); }
