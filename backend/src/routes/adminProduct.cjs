...
router.post('/products/upload', checkAdminAuth, uploader.single('file'), (req, res) => {
  try {
    ...
    console.log('[adminProduct] Uploaded:', filename);
    return res.json({ ok: true, filename, url });
  } catch (err) {
    console.error('[adminProduct] upload error:', err && err.message ? err.message : err);
...
// -------------------
// POST /products -> create (uses Product.create(), no collection.insertOne)
// -------------------
router.post('/products', checkAdminAuth, async (req, res) => {
  try {
    ...
    images = images.map(img => {
      ...
      if (typeof img === 'object') return { filename: img.filename || (img.url && String(img.url).split('/').pop()), url: img.url || img.path || null };
      return null;
    }).filter(Boolean);
    ...
    // Normalize candidate slug; model pre-validate will ensure final uniqueness
    ...
    try {
      ...
      // If duplicate on slug, try a retry with regenerated unique slug
