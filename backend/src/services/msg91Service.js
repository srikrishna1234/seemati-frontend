 ...
 * Minimal MSG91 OTP v5 wrapper: sendOtp + verifyOtp.
 * Uses MSG91_AUTH_KEY, MSG91_TEMPLATE_ID (optional), MSG91_COUNTRY_CODE (default 91).
 */
...
async function sendOtp(phone) {
  if (!MSG91_AUTH_KEY) return { success: false, error: 'MSG91 auth key missing' };
  ...
  try {
    ...
    const respBody = err.response ? (typeof err.response.data === 'object' ? err.response.data : String(err.response.data)) : null;
    return { success: false, error: err.message || 'Request error', raw: respBody || null };
  }
}
